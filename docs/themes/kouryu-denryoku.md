...
??? note "Level 2: 数学的背景 🔬"
    **複素電力とポインティングベクトル**
    ...
    複素電力 \( \dot{S} = \dot{V} \cdot \dot{I}^* = P + jQ \) [VA]
    - \( P \)（実部）= 有効電力 [W]：抵抗で消費
    - \( Q \)（虚部）= 無効電力 [var]：L/Cで蓄放

    電流の共役 \( \dot{I}^* \) を使う理由：電圧と電流の位相差を正しく扱うため。
    \( \dot{V}\dot{I} \) ではなく \( \dot{V}\dot{I}^* \) とすることで、遅れ負荷を \( Q > 0 \)、進み負荷を \( Q < 0 \) と符号で区別できる。
