...
!!! abstract "📍 学習マップ上の現在地"
    **前提** → [トランジスタ・FET](transistor.md)　→　**[オペアンプ]（現在地）**　→　**次** → [計器の原理と測定法](keiki.md)
...
| ゲイン範囲 | $|A_v|$ は 0 以上（$R_f=0$ で **ゲイン 0**＝出力ゼロ・増幅としては $R_f > 0$ で $|A_v| > 0$） | $A_v \geq 1$（1 未満にできない。$R_f = 0$ または $R_i = \infty$ で $A_v = 1$ のバッファ動作） |
...
**→ 次:** [計器の原理](keiki.md)　｜　[電子理論](denshi-riron/index.md)　｜　[公式集](../reference/formulas.md)